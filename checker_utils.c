...
/*   Updated: 2022/03/04 14:41:34 by bepifani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_import(char *st, char *str, int i, int j) //ft_import
{
	...
	zh[0] = 0;
	newline = malloc(sizeof(char) * (ft_strlen(str) + ft_strlen(st) - j + 1));
	if (!newline)
		ft_error(NULL, 1);
	while (zh[0] < i)
	{
...
int	ft_checker_for_pipes(t_info *info) //ft_check_pipes
{
	...
	{
		j = i;
		if (ft_where_quot(info->line[i], 0) == 0 && info->line[i] == '|')
		{
			flag = 0;
